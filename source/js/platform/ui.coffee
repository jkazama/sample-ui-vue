...
UI.Select.values = (list, defaultLabel = null, valueField="value", textField="text") ->
...
UI.Select.calendarDay = (year, month, defaultLabel = null) ->
  ...
  UI.Select.values list, defaultLabel

# 指定した年に応じた月一覧(text/value)を返します。
UI.Select.calendarMonth = (defaultLabel = null) ->
...
UI.Select.calendarYear = (defaultLabel = null, size=10, max=null, reverse = false) ->
...
UI.Select.calendarHour = (defaultLabel = null) ->
  list = _.collect [1..24], (v) -> {text: v, value: v}
  UI.Select.values list, defaultLabel

# 時間一覧(text/value)を返します。(12時間)
UI.Select.calendarHourHalf = (defaultLabel = null) ->
  list = _.collect [1..12], (v) -> {text: v, value: v}
  UI.Select.values list, defaultLabel

# 分一覧(text/value)を返します。
UI.Select.calendarMinute = (defaultLabel = null) ->
